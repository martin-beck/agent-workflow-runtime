#!/usr/bin/env python3
"""Additive offline OpenCode-compatible adapter for AR-0075."""
import hashlib
import json
import re


class OpenCodeAdapterError(ValueError): pass
def digest(value): return "sha256:"+hashlib.sha256(json.dumps(value,sort_keys=True,separators=(",",":")).encode()).hexdigest()
class ExecutableOpenCodeAdapter:
    provider="opencode-compatible"; capabilities=("request","stream","interrupt")
    def __init__(self): self.state="new"; self.events=[]; self.binding=None
    def admit(self,session_id,lease_id):
        if self.state!="new" or not re.fullmatch(r"SES-[A-Z0-9-]{1,48}",session_id) or not re.fullmatch(r"LSE-[A-Z0-9-]{1,48}",lease_id): raise OpenCodeAdapterError("invalid admission")
    ...
    def request(self,request_digest):
        if self.state!="admitted" or not re.fullmatch(r"sha256:[0-9a-f]{64}",request_digest): raise OpenCodeAdapterError("invalid request")
        self.state="active"; return self._event("request",{"request_digest":request_digest})
    def interrupt(self):
        if self.state!="active": raise OpenCodeAdapterError("invalid interruption")
        self.state="interrupted"; return self._event("interrupt",{})
    def close(self):
        if self.state not in {"admitted","active","interrupted"}: raise OpenCodeAdapterError("invalid close")
